...
if __name__ == "__main__":
    ...
    ### BEGIN PHASES ###
    # 1) Disc
    ax.fill_between([0.16, 0.52], y_bins[0], y_bins[1]-buffer, color=cmap([0.8]), alpha=0.8)
    ax.fill_between([0.63, 1.1], y_bins[0], y_bins[1]-buffer, color=cmap([0.8]), alpha=0.8, label="Disc") # 'lightgreen'
    #ax.fill_between([0.84, 1.1], y_bins[0], y_bins[1]-buffer, color=cmap([0.8]), alpha=0.8)
    i += 1
    # 2) Spiral arms
    ax.fill_between([0.33, 1.1], y_bins[1], y_bins[2]-buffer*2, color=cmap([0.4]), alpha=0.8, label="Spiral Arms") # color='cornflowerblue'
    #ax.fill_between([0.93, 1.1], y_bins[1], y_bins[2]-buffer*2, color=cmap([0.4]), alpha=0.8)
    i += 2
    # 3) Bar
    ax.fill_between([0.45, 0.51], y_bins[2], y_bins[3]-buffer*20, color=colors[i], alpha=0.8, label="Bar")
    ...
    # Plot accrate lines
    ax.fill_between(min_max_time, accrate_min, accrate_max, color='cornflowerblue', alpha=0.2) # min-max region
