...
from read_arrays_from_csv import bhl_object_list
import numpy as np
...
a.fill(0.015)
axs[2].plot(BHL.average_times, a, color=c[4], linestyle='solid', label=l[4])
...
for BHL in bhl_object_list:
    ...
    axs[1].plot(BHL.average_times, BHL.average_cinfinity, color=c[i], linestyle='dashed', label=l[i])
